...
function render(items){const el=document.getElementById('results');el.innerHTML=items.map(card).join('')||'<div class="status">无结果</div>';el.querySelectorAll('img').forEach(im=>{im.onerror=()=>im.style.visibility='hidden';});}
